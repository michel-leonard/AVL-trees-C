...
// https://github.com/michel-leonard/AVL-trees-C/blob/main/integer_keys/bst.c

struct bst_entry *bst_at(struct bst_manager *const manager, const int key) {
...
void bst_rm(struct bst_manager *const manager, const int key) {
	...
	if (manager->root) {
		...
		for (;;) {
			if (key != nodes[0]->entry.key) {
				nodes[0] = nodes[0]->rel[key > nodes[0]->entry.key];
				if (nodes[0] == 0) {
					manager->affected = 0;
					return;
				}
